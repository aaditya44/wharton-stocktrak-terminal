...
PORTFOLIO = [
    ...
    ("ITA / LMT", "Defense", "Thematic", "ITA -$85, LMT +$13 at the 09-18 close"),
    ("VTI / EFA", "Broad index ETFs", "Diversifiers", "VTI +$12, EFA -$7 at the 09-18 close"),
    ("2.75% Feb-28 Treasury", "Rates", "Reserve precursor", "50 units (~$49k) bought by Aaditya 09-16; -$70 at the close; the reserve sleeve starts here"),
    ("HCWC (4,000 sh)", "Merger event", "Frozen by the platform", "HOST conversion unprocessed and quoteless at the 09-18 close; carried at $55,080 cost; sells at market the moment a real quote exists"),
    ("Cash $11,650.80", "Dry powder", "Reserve / deployment capacity", "After the 09-18 client deployment and INTC exit; 37/200 trades used"),
]
...
IDEAS = [
    ...
    dict(t="Energy via CVX", theme="Energy", thesis="Crude escalation premium; the XOM leg was cut 09-17 when its thesis broke, exposure rebuilt through client-suitable CVX on 09-18",
         catalyst="Geopolitics, OPEC", entry="CVX 47 sh @ $210.61 done 09-18", exit="De-escalation headlines or client-model rotation",
         conf="Medium", downside="Gap down on any ceasefire headline"),
]
...
TIPS = {
 ...
 "sleeves": "What: the portfolio is split in two: a GROWTH sleeve (stocks, aims to beat inflation) and a RESERVE sleeve (Treasury bonds, aims to guarantee the ten $50,000 payments). Why: Laura's problem is certainty of future payments, not maximum return. Limitation: figures are from the final 2026-09-19 diligence report; live coverage is projected, not guaranteed.",
...
TRADES = [
    ...
    ("2026-09-18 late session ET", "INTC", "SELL", "50 sh @ $108.43 ($5,396.50 net)", "Conf 36CBB28B. Full exit of the 09-17 Exp 9 entry - stayed weak and sits outside the client-suitable sleeve; cut per the standing pre-close rule. Realized -$104 incl. commissions (-1.9%). Rule-compliant: bought 09-17, sold 09-18. Semis exposure consolidated into NVDA. Cash $11,650.80, trades 37/200"),
]
...
DECISIONS = [
    ...
    ("2026-09-17", "LESSON", "Near-close large order (SOXL 116) qty-reduced by the platform", "Standing rule: size orders earlier in the session near the close", "Trade log 2026-09-17 ~15:40 ET", "Trading Notes"),
    ...
    ("2026-09-18", "DESIGN", "Two-sleeve architecture: growth sleeve + Treasury reserve sleeve; reserve target $430-440k nominal ladder by start-2033; glidepath ~80/20 toward 60/40 by 2031; reserve-coverage ratio + 3 de-risk triggers first-class", "FINAL diligence 2026-09-19: certainty comes from segregating and date-matching the reserve - 60/40 still misses on 4.6-9.3% of paths, a crash year ~29-30%", "Portfolio risk tab; Case tab", "IPS + Final Report"),
    ("2026-09-19", "DESIGN", "Final 10-hour due-diligence report integrated (supersedes the v1 draft): verified public-record findings with LinkedIn namesake warning; reserve PV table $397.6-439.3k by discount rate, sized at $430-440k (3.0-3.5%); Monte Carlo v2 with fat tails + 2008 crash stress; year-by-year glidepath + IPS rebalancing rules; co-sponsor $100-250k with explicit <$50k severe-drawdown downside; USD/TWD ~32.2-32.4 noted as FX risk; 200-trade allotment confirmed", "MC v2: 100% equity misses the reserve on 14.8-21.4% of paths, 60/40 on 4.6-9.3%, one crash year ~29-30% - allocation alone cannot deliver the required certainty", "Case tab; Portfolio risk tab; Client mode", "IPS + Final Report"),
    ...
    ("2026-09-18", "CLOSE", "Final pre-close decision: NO further action - no sell triggers fired, no new entry signals, and any new entry would carry 2.5-day weekend gap risk", "Discipline over activity: the algorithm choosing not to trade is also the model working", "Trading-agent verified close report 2026-09-18", "Trading Notes"),
    ("2026-09-18", "STATE", "Verified Sep 18 close: displayed portfolio $302,155 (includes HCWC frozen at $55,080 cost - HOST conversion still unprocessed and quoteless at close); tradeable book ~$247,075 plus cash $11,650.80; open P&L +$2,737; 37/200 trades used", "Frozen HCWC inflates the displayed total; per-position close marks logged on the Case tab", "Case tab close snapshot; Portfolio tab", "Trading Notes"),
]
...
CASE_STUDY = [
    ...
    "DELIVERABLES (official schedule, all 5:00 PM ET): Trading Notes Analysis - 3 notes showing decisions that supported/tested/refined strategy - due Fri Oct 23, submissions open Mon Oct 12. Investment Policy Statement due Fri Nov 6, submissions open Mon Oct 26; trading ends and the portfolio freezes Nov 6, strategy may not be revised after. Final Report due Fri Dec 4, submissions open Nov 9. School Documentation also due Fri Dec 4, submissions open Nov 9. Team roster due Fri Oct 9; detailed Final Report requirements arrive at the start of Week 7.",
    "UNKNOWN PRESERVED (final report open items): the approved WInS securities universe, exact IPS/Trading Notes requirements, and evaluation criteria live on SurveyMonkey Apply (needed from Aaditya); WInS minimum-activity rules, position limits, and team starting cash sit on the team's WInS Portfolio Summary page. The 200-trade allotment IS confirmed (WInS user guide) and rankings are explicitly just for fun. Confirm before assuming every screened symbol is WInS-eligible.",
    "FINAL - DUE-DILIGENCE REPORT 2026-09-19 (full 10-hour program; supersedes the 09-18 working draft): Laura Gao is a REAL, verified person - Wuhan-born, raised Coppell TX, Wharton Statistics 2018 (summa cum laude), Twitter PM 2018-2020, The Wuhan I Know (2020), Messy Roots (2022, #9 Indie Bestseller), Kirby's Lessons for Falling (in Love) (2025), comics professor at California College of the Arts. Multi-stream creator income (books, speaking/workshops, CCA teaching, Patreon, LG Studios design) supports the case's living-expenses stipulation. NAMESAKE WARNING: linkedin.com/in/lauragao and linkedin.com/in/laura-gao-58117711 are DIFFERENT people - no LinkedIn-sourced claims are attributed to her. The financial plan is the fictional layer. Required blended return ~1.9% - a certainty problem, not a return chase. Reserve $430-440k at a conservative 3.0-3.5% discount (PV table $397.6-439.3k across 3.0-5.5% yields), nominal date-matched Treasury ladder - the liability carries no inflation risk. Monte Carlo v2 (20k paths, fat tails): 100% equity misses the reserve on 14.8-21.4% of paths, 60/40 on 4.6-9.3%, and one 2008-style crash year pushes it to ~29-30% (median headroom ~$48k) - so the reserve is SEGREGATED and funded first; only the facility contribution rides equity outcomes. Facility headroom ~$155-220k on institutional assumptions (Vanguard US equities 4.2-6.2%, JPM ~6.7%). Co-sponsor range $100-250k, the floor fundable in ~70-77% of simulated paths, with the stated downside below $50k in a severe 2029-2032 drawdown.",
"RESERVE GLIDEPATH (final report, for the IPS): 2027-28 ~80/20, seed the reserve sleeve with intermediate Treasuries/IG (~$90-100k target); 2029 step toward 70/30, add on equity strength, first annual coverage check; 2030 ~65/35, coverage measured at the conservative PV; 2031 ~60/40, publish the co-sponsor range, shorten reserve duration, coverage must be >= 1.0 or de-risk immediately; 2032 reserve fully funded (~$430-440k) in short Treasuries/bills; start-2033 convert to a date-matched nominal ladder covering 2033-2042; 2033-2042 each January tranche held in bills from 12 months ahead as the ladder winds to zero. Rebalancing: annual January review, rebalance on >5pp sleeve drift, immediate de-risk if coverage < 1.0, re-risk only above 1.1 with the facility below base case. Every trigger maps to a WInS Trading Note.",
"MARKET ENVIRONMENT (2026-09-18, final report): the Fed hiked 25bp to 3.75-4.00% on 09-16 (first hike since July 2023, at least one more signaled); CPI ~3.4%; 10Y Treasury ~5.0% (19-year high), 30Y ~5.34%, curve nearly flat - the reserve is historically cheap to fund on this tape. Institutional 10-year assumptions: Vanguard US equities 4.2-6.2%, JPM ~6.7%; the IPS blends 5-7%. Taiwan grounding: Taipei centre ~US$6,100/sqm at USD/TWD ~32.2-32.4, with the TWD ~10% weaker over 12 months - FX uncertainty on a TWD-priced facility is noted as a risk factor, not hedging advice. At real residency benchmarks (Pier-2, Grand View), $50k/yr plausibly funds stipends and living support for a rotating cohort plus part-time administration.",
...
def conf_badge(c):
    col = {"High":"#2da44e","Medium":"#bf8700","Low":"#cf222e","Research":"#8250df","Closed":"#6e7781"}[c]
...
.card{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px 18px;margin-bottom:16px;overflow-x:auto}
...
@media (pointer:coarse){.info{width:20px;height:20px;line-height:18px;font-size:12px}}
@media (max-width:700px){header{padding:12px 14px}section{padding:14px}.card{padding:12px}table{font-size:12px}nav{overflow-x:auto;white-space:nowrap}}
...
<div class="warn">FINAL - client due-diligence report 2026-09-19 (full 10-hour program; supersedes the v1 draft): Laura's mandate is LIABILITY-DRIVEN. Required blended return is only ~1.9% to fund the ten $50k payments - the problem is CERTAINTY, not growth. Architecture: a GROWTH sleeve and a SEGREGATED RESERVE sleeve, gliding ~80/20 (2027-28) to ~60/40 (2031); the reserve is fully funded by 2032 and converted to a date-matched nominal ladder at start-2033. Reserve sized at a conservative 3.0-3.5% discount = $430-440k (rate-dependent PV $397.6k-$439.3k). Monte Carlo v2 (20k paths, fat tails + 2008 crash stress): even 60/40 misses the reserve on 4.6-9.3% of paths and a severe crash year pushes that to ~29-30% - so the reserve is segregated and funded first, and crash risk lands only on the facility contribution. Facility headroom ~$155-220k; 2031 co-sponsor range $100-250k with the explicit severe-drawdown downside below $50k.</div>
...
<li><b>IPS rebalancing rules (final report):</b> annual January review; rebalance on sleeve drift &gt; 5pp; immediate de-risk if reserve coverage &lt; 1.0 at a conservative discount rate; re-risk only when coverage &gt; 1.1 and the facility outlook is below base case.</li>
...
<div class="warn">Positions verified through the 2026-09-18 close. Largest structural risks: HCWC event risk (frozen by the platform at $55,080 cost with no live quote; sells at market once a real quote exists), overnight gaps in the leveraged sleeve (TQQQ), weekend gap risk across the whole book. Positions bought today cannot be sold today - gap risk is undiversifiable within a session.</div>
